...
public class MultiLazyDataSourcePool {
    ...
    private static final RemovalListener<String, ManagedDataSourcePool> poolRemovalListener = (s, managedDataSourcePool, removalCause) -> {
        ...
        if(managedDataSourcePool != null && managedDataSourcePool.getPool()  != null){
            ...
            if(managedDataSourcePool == null)  log.error("unable to close the pool because the managed data source is null for " + s);
            else if(managedDataSourcePool.getPool() == null) log.error("unable to close the pool because the pool is null for " + s);
        }
    };
     ...
     *
     * If relevant values change, the old data source is closed and a new one is returned.
     * Be aware that the data source will thrash and degrade performance if this method is constantly called with
     * params that contain differing CP settings.
     *
     * When a DataSource is requested, the user-supplied function is invoked to return the id for the pool you'r requesting.
     * If the requested pool currently exists, the HikariParams associated with that pool are checked for any differences
     * from the pool's last configuration. If there are changes, the original pool is shutdown and replaced with a new pool
     * using the new configuration. If there are not changes, the original pool is returned.
     *
     * This class maintains a thread-safe cache of pool IDs to pools and their configurations.
     * Pools will be evicted from the cache if enough time passes without them being used.
     *
     * @param params Request params. These should contain parameters that map to the HikariParams model. Any missing hikari params will default to natural hikari defaults.
     * @return A reference to the DataSource defined by the params
     */
    public static DataSource getDataSource(final Map<String, String> params, PoolIdSupplier poolIdSupplier, ConnectorLogging clog, ConnectorMessage cm) {

        final HikariParams hikariParams = new HikariParams(params);

        // We have a cache that maps pool Ids to pool locks, and a cache that maps pool ids to the pools themselves.
        ...
        ManagedDataSourcePool managedDataSourcePool = pools.get(poolId,
                ...
                { clog.info(cm,"No pool found in the cache -- creating a new pool for id: " + id);
                    return new ManagedDataSourcePool(hikariParams, setupConnectionPool(hikariParams, clog, cm), new ReentrantLock());
                }
        );
        ...
        // Each poolId's operations need to be synchronized, so first acquire the lock for this pool.
        ...
        try {
            ...
            // Reset the connection pool if the new HikariParams are different from the old ones.
            if(!hikariParams.equals(managedDataSourcePool.hikariParams)){
                ...
                clog.info(cm, "The params are for a new pool or have NOT changed for the existing pool. Using pool with id: " + poolId);
    ...
    /**
     * Sets up a Hikari connection pool with the params passed in, and then runs the supplied consumer with the data source.
     */
    private static HikariDataSource setupConnectionPool(HikariParams hikariParams, ConnectorLogging clog, ConnectorMessage cm){
    ...
    /**
     * Models for the parameters that can be used to control a Hikari DataSource pool.
    ...
    public static class HikariParams {
        ...
        public HikariParams(Map<String, String> params){
            ...
            this.localCpMinimumIdle = toIntExact(getLongParamOrDefault(params, "localCpMinIdle", this.localCpMaxPoolSize.longValue()));
